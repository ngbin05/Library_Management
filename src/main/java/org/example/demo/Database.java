...
public class Database {
    ...
    public static boolean isBookExistByIsbn(String isbn, String title) {
        ...
        if (isbn == null || isbn.equals("Unknown ISBN")) {
            return isBookExistByTitle(title);  // Kiểm tra theo tên sách
        }
        ...
        try (Connection conn = connect();
            ...
            pstmt.setString(1, isbn);  // Kiểm tra theo ISBN

            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
                int count = rs.getInt(1);
                return count > 0;  // Nếu có ít nhất 1 sách trùng ISBN, trả về true
            }
        } catch (SQLException e) {
            System.out.println("Error checking ISBN: " + e.getMessage());
        }
        return false;  // Không có sách nào trùng ISBN
    }
    ...
    private static boolean isBookExistByTitle(String title) {
        ...
        try (Connection conn = connect();
            ...
            pstmt.setString(1, title);  // Kiểm tra theo tên sách

            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) {
                int count = rs.getInt(1);
                return count > 0;  // Nếu có ít nhất 1 sách trùng tên sách, trả về true
            }
        } catch (SQLException e) {
            System.out.println("Error checking book title: " + e.getMessage());
        }
        return false;  // Không có sách nào trùng tên sách
    }
